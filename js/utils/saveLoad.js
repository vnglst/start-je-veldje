...
function exportGameData() {
  ...
  showMessage(`Speldata geëxporteerd! Bestand: start-je-veldje-save-dag${gameState.day}-${gameState.season.toLowerCase()}.json`, "success");
}
  ...
  input.onchange = function(event) {
    ...
    reader.onload = function(e) {
      try {
        ...
        if (typeof importedGameState.money !== "number" || 
        ...
        try {
          ...
          if (!gameState.iceCream) {
            gameState.iceCream = {
              vanilla: 0, strawberry: 0, apple: 0, carrot: 0, corn: 0, berry_mix: 0,
              tropical_paradise: 0, autumn_harvest: 0, rainbow_deluxe: 0,
              veggie_surprise: 0, summer_festival: 0, golden_treasure: 0,
            };
          }
          ...
          if (metadata) {
            showMessage(`Speldata succesvol geïmporteerd! Dag ${metadata.day}, ${metadata.season}, €${metadata.money} 💾`, "success");
          } else {
            showMessage(`Legacy speldata succesvol geïmporteerd! Dag ${gameState.day}, ${gameState.season} 💾`, "success");
          }
